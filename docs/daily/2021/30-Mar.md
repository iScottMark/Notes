...
- 完成 indice - [M/H] 及打乱顺寻的 xgb 分类任务
- "BL2740", "BL3096" 和 "MgII 2800" 属于重要特征
...
- 按照小于流量中值的 7% 添加高斯白噪声
- 实验结果
  - fluxes - Teff 打乱顺序后，拟合弥散变大，之前的老问题继续存在
  - fluxes - [M/H] 打乱顺序后，分类结果较差，但 "2500" 和 "3550" 这两根线贡献度较大

**NumPy**

- 基础知识 - ndarray 核心数据对象、数组的属性、数组的操作、数组的索引（尤其是 `np.where()`）
