...
- 使背景图片更加均匀（透明度根据背景图片具体调整，通常背景浅透明度小；背景深透明度大）
- 使文字与背景形成一定的反差（因而选择与文字颜色反差较大的蒙版颜色）

**弥补素材尺寸**

- 对于非“全图型”蒙版，可以使用“渐变型蒙版”
- 从图片的主色调中取出蒙版的颜色
- 应用场景
  - 使得非全图的图片边界柔和过渡至文字部分
  - 使得全图中的关键部分不被遮盖，且展现文字内容

**凸显焦点内容**

- 比如一张图表里，只想强调某个数据，可以用蒙版遮盖掉其余部分
...
- 全背景图型
  - 较为均匀/有蒙版覆盖，文字居中放置
  - 有突出的部分（如右半部分有人像），则在左半部分放置文字
- 非全背景图型
  - 文字放置空白处

::: tip
小 trick :bulb:：将主标题与人像眼睛平齐，会更加凸显主标题
:::

**位置**

- 结合前面两者，确定文字与背景图的位置关系
